...
# Function to create a unix user
def create_unix_user(username: str):
    # generate a random password, no user will ever need to know their password
    random_pass = secrets.token_hex(32)

    # Create the user in the mammoth cluster users
    command = f"sudo useradd {username} -p {random_pass} -g {MAMMOTH_USERS_GROUP}"
    subprocess.run(command, check=True, shell=True)
...
def sync_cloudflare_access_policy(users: list[str]):
    ...
    policy = client.zero_trust.access.policies.update(
        ...
        account_id=CLOUDFLARE_ACCESS_POLICY_ID,
        include=[{
            "email": {
    ...
    print(f"Updated access policy with id: {policy.id}")
